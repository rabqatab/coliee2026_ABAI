"""Per-stage intermediate retrieval metrics (from w1_e_stage_metrics.py).
...
Quantifies how ranking quality evolves stage to stage (Reviewer 1 request).

Run:  uv run python scripts/experiments/stage_metrics.py
Writes: output/experiments/stage_metrics.json  (+ legacy output/w1/e_stage_metrics.json)
"""
import json
...
LEGACY_OUT = REPO / "output" / "w1" / "e_stage_metrics.json"
RECALL_KS = [5, 10, 50, 100, 200]
NDCG_KS = [5, 10]


def load_pickle(name: str):
    with open(PIPELINE_CACHE / name, "rb") as f:
...
def print_table(results, avg="micro"):
    cols = [f"R@{k}" for k in RECALL_KS] + [f"NDCG@{k}" for k in NDCG_KS] + ["MAP"]
    keys = [f"recall@{k}" for k in RECALL_KS] + [f"ndcg@{k}" for k in NDCG_KS] + ["map"]
...
def main():
    ...
    results = {stage: compute_metrics(ranked, labels) for stage, ranked in stages.items()}
    ...
    print(f"[1] BM25-RRF Recall@200 (micro) = {r200:.4f} (expected ~0.578) -> "
          f"{'PASS' if check1 else 'CHECK'}")
    ce_ndcg10 = results["crossencoder"]["ndcg@10"]["micro"]
    other_max = max(results[s]["ndcg@10"]["micro"] for s in results if s != "crossencoder")
    check2 = ce_ndcg10 >= other_max
    best = max(results, key=lambda s: results[s]["ndcg@10"]["micro"])
    print(f"[2] Cross-encoder NDCG@10 (micro) = {ce_ndcg10:.4f}, max other = {other_max:.4f}, "
          f"highest stage = '{best}' -> {'PASS' if check2 else 'CHECK'}")
    ...
    path = write_result("stage_metrics", payload, script="experiments/stage_metrics.py")
    LEGACY_OUT.parent.mkdir(parents=True, exist_ok=True)
    LEGACY_OUT.write_text(json.dumps(payload, indent=2))
    print(f"\nSaved {path}\nSaved {LEGACY_OUT}")
