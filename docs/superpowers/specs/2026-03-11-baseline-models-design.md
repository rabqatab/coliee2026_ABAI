...
- **SAILER**: Use pre-trained HuggingFace checkpoint (`nguha/sailer`), not trained from scratch
...
@dataclass
class Dataset:
    corpus: dict[str, str]             # {doc_id: text} for all ~7,708 docs
...
**Loading logic:**
1. Read all `.txt` files from `data/task1/task1_train_files_2026/`
...
3. Split queries (keys of labels dict) into train/val:
   ...
   - Last 20% of queries become validation set
   - Remaining 80% become training set
   - This mimics COLIEE's chronological train/test split
4. Corpus includes ALL documents (both query and candidate pools)

**Caching:** Corpus text is loaded once and kept in memory (~1-2 GB for 7,708 docs).
...
class SAILEREncoder:
    """
    Downloads nguha/sailer from HuggingFace.
    Encodes documents with structure-aware segmentation.
...
SAILER segments documents into structural parts (Facts, Issues, Analysis, Conclusion) before encoding. The pre-trained model handles this internally via its tokenization strategy.

**Caching**: Embeddings for all ~7,708 docs saved to disk as `.npy` files. Computed once, loaded on subsequent runs.
...
| `dense_cosine` | Fine-tuned bi-encoder cosine similarity |
| `query_len` | Query word count |
| `doc_len` | Candidate word count |
| `len_ratio` | query_len / doc_len |
| `paragraph_count` | Number of paragraphs in candidate |
| `shared_top_k` | How many of this candidate's BM25 top-50 overlap with query's top-50 |
| `year_proximity` | Absolute difference in case year (if extractable) |

#### Post-Processing Heuristics
From the paper's description of "heuristic strategies based on common properties of relevant cases":
1. **Shared-neighbor boost**: If candidate C appears in BM25 top-50 of multiple other top-ranked candidates, boost C's score
2. **Score gap filtering**: If there's a large gap between candidate N and N+1, cut at N
...
2. **Case-Charge edges**: Extract statute/charge references from case text (reuse `src/graphrag/extract_regex.py`), create edges from case nodes to charge/statute nodes
3. **Charge-Charge edges**: Two charges are connected if they co-occur in the same case above a threshold frequency

Node types:
- Case nodes (~7,708)
- Charge/Statute nodes (variable, estimated ~500-2,000)
...
- Charge nodes: aggregate embeddings of associated cases
...
- Run GNN forward pass using edges available at test time (Case-Charge edges can be computed for test cases; Case-Case edges only available for training)
...
============================================================
Baseline                      F1      Prec    Rec     Thresh
------------------------------------------------------------
BM25 (vanilla)               0.182   0.145   0.244   0.34
JNLP (BM25+SAILER+LGBM)     0.312   0.298   0.327   0.52
TQM (LTR fusion)             0.351   0.402   0.312   0.48
UMNLP (propositions+NN)      0.338   0.356   0.322   0.55
CaseLink (GNN)               0.271   0.265   0.278   0.41
GraphRAG (ours)              ----    ----    ----    ----
============================================================
```

(Values above are illustrative, not predictions.)
...
transformers = ">=4.30"           # for SAILER checkpoint
torch-geometric = ">=2.4"
torch-scatter = "*"
torch-sparse = "*"
```
...
### Dependency Notes
- `torch` is needed by three baselines (UMNLP FF NN, TQM bi-encoder fine-tuning, CaseLink GNN)
- `sentence-transformers` is needed by TQM and UMNLP (paragraph/sentence similarity)
- `torch-geometric` is only needed by CaseLink
- SAILER checkpoint loaded via `transformers` AutoModel
...
|------|--------|------------|
| SAILER checkpoint incompatible with transformers version | Blocks JNLP baseline | Test download early; fall back to Legal-BERT if needed |
